...
SPOTIPY_REDIRECT_URI = 'https://studysift-jbyhh4glfowhcs8xszu9xr.streamlit.app'  # Replace with your Streamlit app URL
...
def authenticate_spotify():
    ...
    if token_info is None:
        ...
        if auth_code:
            ...
            if token_info:
                ...
                new_query_params = st.experimental_get_query_params()
    ...
    if token_info:
        ...
        st.error("Please authorize your Spotify account to use this app.")
        st.stop()
...
def get_track_info_concurrent(track_ids, sp):
    """
    Retrieves track information (name, artists, and Spotify URL) for a list of track IDs using threading for concurrency.
    """
    track_info_list = []
    lock = threading.Lock()
...
# --------------------------- UI and Main Functionality --------------------------- #

def main():
    st.set_page_config(page_title="Echo (AI-Powered Playlists)", layout="wide")
    st.title("Echo (AI-Powered Playlists)")

    # Sidebar for account disconnection
    with st.sidebar:
        if 'token_info' in st.session_state and st.session_state['token_info']:
            if st.button("Disconnect Spotify Account"):
                disconnect_spotify()
    ...
    if 'playlist_tracks' in st.session_state:
        with st.expander("### Your Playlist Tracks"):
            df_tracks = pd.DataFrame(st.session_state['playlist_tracks'])
            if not df_tracks.empty:
                # Create clickable links for track names
                df_tracks['Track'] = df_tracks.apply(
                    lambda row: f"[{row['name']}]({row['url']})", axis=1
                )
                df_tracks['Artists'] = df_tracks['artists']
                st.markdown(df_tracks[['Track', 'Artists']].to_markdown(index=False), unsafe_allow_html=True)
            else:
                st.write("No tracks to display.")
...
Welcome to **Echo (AI-Powered Playlists)**! We value your privacy and are committed to protecting your personal information. This Privacy Policy outlines how we collect, use, disclose, and safeguard your data when you use our Streamlit application to create personalized Spotify playlists.

#### **2. Data Collection**

When you use Echo, we collect the following types of information:

- **Spotify Data:**
  - **Access Tokens:** To authenticate your Spotify account and create playlists on your behalf.
  - **Profile Information:** Your Spotify user ID and display name.
  - **Playlist Data:** Information about your existing playlists to curate new ones.
  - **Track Information:** Details about tracks, including names, artists, and audio features.

- **User Inputs:**
  - **Preferences:** Information you provide regarding Genre, Mood, Energy Level, and any additional preferences to tailor the playlist to your liking.

#### **3. Data Usage**

We use the collected data solely for the purpose of:

- **Creating Playlists:** Generating personalized Spotify playlists based on your specified preferences.
- **Enhancing User Experience:** Analyzing your preferences to improve playlist recommendations.

#### **4. Data Sharing**

Your data is **never shared** with third parties. All processing and data handling occur within the Echo application to ensure your information remains secure and private.

#### **5. Data Retention**

- **Session-Based Storage:** All data collected during your session is stored temporarily to facilitate playlist creation.
- **No Long-Term Storage:** Once you disconnect your Spotify account or end your session, all associated data is immediately deleted from our systems.
...
- **How to Disconnect:**
  ...
  - Click on the **"Disconnect Spotify Account"** button to revoke access and delete your data.

#### **7. Security**

We implement industry-standard security measures to protect your data from unauthorized access, alteration, disclosure, or destruction. This includes:

- **Encrypted Data Transmission:** All data exchanged between your device and our servers is encrypted.
- **Secure Storage:** Sensitive information, such as access tokens and API keys, is stored securely using Streamlit's secrets management and is not exposed to the client side.

#### **8. Changes to This Privacy Policy**

We may update our Privacy Policy from time to time. Any changes will be reflected within the application, and the **Effective Date** will be updated accordingly. We encourage you to review this policy periodically to stay informed about how we are protecting your information.

#### **9. Contact Us**

If you have any questions, concerns, or requests regarding this Privacy Policy or our data practices, please reach out to us once you have established a preferred contact method.

**Consent:**

By using this application, you consent to the terms outlined in this privacy policy.
        """)

def disconnect_spotify():
    token_info_key = 'token_info'
    if token_info_key in st.session_state:
        del st.session_state[token_info_key]
    st.success("Your Spotify account has been disconnected.")
...
def process_playlist_generation(
    ...
    try:
        ...
        if new_playlist_url:
            ...
            # Store track information in session state to display later
            st.session_state['playlist_tracks'] = [
                {'name': track['name'], 'artists': track['artists'], 'url': track['url']} for track in track_info_list if track['id'] in filtered_ids
            ]
        else:
